    ...
    return {
      ...
      documentOptions: {
        ...
        requestFunction: request => {
          ...
          return this.$userRepository
            .getDocuments(this.$store.state.auth.user._id, params)
            .catch(error => {
      ...
      sharedOptions: {
        ...
        requestFunction: request => {
          ...
          this.loading = true;
          return this.$userRepository
            .getSharedDocuments(this.$store.state.auth.user._id, params)
            .catch(error => {
              console.error('Documents.requestFunction-error:', error);
              this.$snotify.error('Get documents failed', 'Error!');
            });
  ...
  methods: {
    ...
    sharedRowClick(data) {
      this.$router.push({
        name: 'DocumentDetails',
        params: { _id: data.row._id },
        query: { canEdit: data.row.user_document.canEdit },
      });
    },
    onLoaded() {
