  ...
  <section>
    <div class="box box-primary box-solid box-modal">
      <div class="box-header">
        ...
        <div class="box-tools">
          <button class="btn btn-box-tool" title="Collapse" @click="closeModal">
            <i class="fa fa-times" />
          </button>
        </div>
      </div>
                ...
                <div>
                  <div class="btn-group">
                    <button
                      ...
                      aria-expanded="false"
                    >
                      <a class="fa fa-pencil" /><span class="caret" style="margin-left: 5px;" />
                    </button>

                    <ul class="dropdown-menu">
                      <li @click="allowEdit(props.row)">
                        <a href="#"
                          ><i
                            v-if="props.row.user_document.canEdit"
                            class="fa fa-check"
                            aria-hidden="true"
                          />
                          Can edit</a
        ...
        >
          ...
          <div slot="body">
            <v-server-table
              ...
              :options="inviteTableOptions"
            >
              <template slot="invite" slot-scope="props">
                <div
                  class="checkbox checkbox-success"
                  @click="removeDuplicate({ childId: props.row._id, canEdit: true })"
                >
                  <input
                    :id="'invite_edit_' + props.row._id"
                    v-model="usersToInvite"
                    type="checkbox"
                    :value="{ childId: props.row._id, canEdit: true }"
                  />
                  <label :for="'invite_edit_' + props.row._id">Edit</label>
                </div>
                <div
                  class="checkbox checkbox-warning"
                  @click="removeDuplicate({ childId: props.row._id, canEdit: false })"
                >
                  <input
                    :id="'invite_view_' + props.row._id"
                    v-model="usersToInvite"
                    type="checkbox"
                    :value="{ childId: props.row._id, canEdit: false }"
                  />
      ...
      <div class="box-footer">
        ...
        <button
          v-permission.enable="['document', 'associateDocument', 'addDocumentUsers']"
          type="button"
...
export default {
    ...
    return {
      ...
      inviteTableOptions: {
        ...
        responseAdapter: response => {
          ...
          return response
            ? { data: response.data.docs, count: response.data.items.total }
            : { data: [], count: 0 };
        },
        uniqueKey: '_id',
      },
  ...
  computed: {
    sharedUserIds() {
      return this.sharedUsers.map(user => {
        return user._id;
      });
    },
  },
  ...
  methods: {
    ...
    inviteUsers() {
      this.loading = true;
      documentService
        .updateDocumentUsers(this.document._id, this.usersToInvite)
        .then(() => {
          this.loading = false;
          this.usersToInvite = [];
    ...
    allowEdit(user) {
      ...
      this.$documentRepository
        .addOneUsers(this.document._id, user._id, { canEdit: true })
        .then(() => {
          this.$refs.shareTable.refresh();
    ...
    allowView(user) {
      this.shareLoading = true;
      this.$documentRepository
        .addOneUsers(this.document._id, user._id, { canEdit: false })
        .then(() => {
          this.$refs.shareTable.refresh();
          this.$snotify.success('User access updated', 'Success!');
        })
    ...
    stopSharing(user) {
      this.shareLoading = true;
      this.$documentRepository
        .removeOneUsers(this.document._id, user._id)
        .then(() => {
          this.$refs.shareTable.refresh();
          this.$snotify.success('User access updated', 'Success!');
        })
