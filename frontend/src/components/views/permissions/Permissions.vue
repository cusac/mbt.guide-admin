<template>
  <section class="content">
    <div class="box box-primary box-solid">
      <div class="box-body">
        <div v-permission.if="['permission', 'createPermission']" :class="addButtonClass">
        ...
        >
          <template slot="beforeBody">
            <tr v-if="loading" class="VueTables__no-results">
              <td class="text-center" colspan="6"><pulse-loader /></td>
            </tr>
          </template>
          <template slot="edit" slot-scope="props">
            <div>
              <a class="fa fa-edit" permission="button" @click.stop="edit(props.row)" />
            </div>
          </template>
    ...
    return {
      ...
      options: {
        ...
        requestFunction: request => {
          ...
          this.loading = true;
          return this.$permissionRepository.list(params).catch(error => {
  ...
  computed: {
    addButtonClass() {
      if (this.permissionTable) {
        return this.permissionTable.count > this.permissionTable.limit
          ? 'shift-add-left'
          : 'add-item';
      } else {
        return 'add-item';
