          ...
          <div class="tab-content content">
            <div id="details" class="tab-pane fade in active">
              <vue-form :state="formstate" class="row" @submit.prevent="onSubmit">
                ...
                <validate class="hide"
                  ><input v-model="permissionUsersUpdated" type="text" name="permissionUsersUpdated"
                /></validate>
                <validate class="hide"
                  ><input
                    v-model="permissionGroupsUpdated"
                    type="text"
                    name="permissionGroupsUpdated"
                /></validate>
              </vue-form>
              ...
              <div class="py-2 text-center row" style="margin-top: 10px">
                <button class="btn btn-danger" @click="deletePermissionModal">
                  Delete Permission
                </button>
                <button
                  class="btn btn-primary"
            ...
            <div id="users" class="tab-pane fade">
              <permission-users v-if="!loading" :permission="newPermission" />
            </div>
            <div id="groups" class="tab-pane fade">
              <permission-groups v-if="!loading" :permission="newPermission" />
            </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    updatePermission() {
      ...
      return permissionService
        .updatePermission(this.newPermission, this.oldPermission)
        .then(() => {
          this.loading = false;
          this.flash = false;
          this.formstate._reset();
    ...
    deletePermission() {
      this.loading = true;
      return this.$permissionRepository
        .deleteOne(this.newPermission._id)
        .then(() => {
          this.loading = false;
          return swal('Deleted!', 'Permission has been deleted.', 'success');
        })
        .then(() => {
          this.loading = false;
