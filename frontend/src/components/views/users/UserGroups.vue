        ...
        <div class="col-sm-5">
          <div class="box box-primary box-solid">
            ...
            <div class="box-body">
              ...
              >
                <option v-for="obj in newUser.groups" :key="obj" :value="obj">
                  {{ obj.group.name }}
                </option>
              </select>
            </div>
...
export default {
  ...
  computed: {
    groupDescription() {
      const group =
        (this.selectedAvailableGroups[0] || {}).group || (this.selectedUserGroups[0] || {}).group;
      if (group) {
  ...
  methods: {
    getAvailableGroups() {
      this.loading = true;
      const userGroupIds = this.newUser.groups.map(object => {
    ...
    updateUserGroups() {
      userService
        .updateUserGroups(this.newUser._id, this.newUser.groups, this.oldUser.groups)
        .then(() => {
          this.loading = false;
          this.dirty = false;
          this.oldUser = _.cloneDeep(this.newUser);
