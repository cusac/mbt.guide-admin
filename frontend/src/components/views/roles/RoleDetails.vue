          ...
          <div class="tab-content content">
            <div id="details" class="tab-pane fade in active">
              <vue-form :state="formstate" class="row" @submit.prevent="onSubmit">
                ...
                <validate class="hide"
                  ><input v-model="roleUpdated" type="text" name="roleUpdated"
                /></validate>
              </vue-form>

              <div class="py-2 text-center row" style="margin-top: 10px">
                <button
                  v-permission.enable="['role', 'updateRole']"
              ...
              <div class="py-2 text-center row" style="margin-top: 10px">
                <button
                  v-permission.enable="['role', 'deleteRole']"
            ...
            <div id="permissions" class="tab-pane fade">
              <role-permissions v-if="!loading" :role="newRole" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    updateRole() {
      this.loading = true;
      return roleService
        .updateRole(this.newRole, this.oldRole)
        .then(() => {
          this.loading = false;
          this.flash = false;
          this.formstate._reset();
        ...
        .catch(error => {
          ...
          if (error.data.message.includes('must be one of')) {
            ...
            this.flashMessage =
              ...
              ' in the mbt.guide client and server config.';
          }
        });
    },
    deleteRoleModal() {
    ...
    deleteRole() {
      this.loading = true;
      return this.$roleRepository
        .deleteOne(this.newRole._id)
        .then(() => {
          this.loading = false;
          return swal('Deleted!', 'Role has been deleted.', 'success');
        })
        .then(() => {
          this.loading = false;
