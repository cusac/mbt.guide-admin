  ...
  <section class="content">
    <div>
      <div class="box box-primary box-solid">
        ...
        <div v-if="ready" ref="imageBox" class="box-body">
          <div v-if="images[0]">
            <carousel-3d
              v-if="enableCarousel"
              ref="carousel"
            ...
            >
              <slide v-for="(image, index) in images" :key="image._id" :index="index">
            ...
            <div class="content-centered">
              <router-link :to="{ name: 'ImageCreate' }">
                <button class="btn btn-primary btn-lg">
                  <i v-permission.enable="['image', 'createImage']" class="fa fa-file-picture-o" />
                  Add Image
                </button>
              </router-link>
              <button
                ...
                @click="openEditModal"
              >
                <i class="fa fa-trash" /> Edit Images
              </button>
            </div>
          </div>
        </div>
    ...
    <modal
      v-if="ready && carousel && enableCarousel && images[0]"
      :adaptive="true"
          ...
          >
            <div class="content-centered" style="height: 100%">
              <img
                ...
                :src="image.imageUrl"
                :width="image.w * 70 * 2"
                :height="(image.h * 70 * 2) / 3"
              />
            </div>

            <svg
              v-permission.enable="['image', 'deleteImage']"
              class="icon icon-edit-remove"
            ...
            >
              <path
                d="M511.921231 0C229.179077 0 0 229.257846 0 512 0 794.702769 229.179077 1024 511.921231 1024
...
export default {
  ...
  methods: {
    getImages() {
      this.loading = true;
      this.$userRepository
        .getImages(this.$store.state.auth.user._id, {})
        .then(result => {
          this.ready = true;
          this.loading = false;
