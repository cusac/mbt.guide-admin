...
export default {
  ...
  created() {
    const promises = [];
    promises.push(
      this.$userRepository.getConnections(this.$store.state.auth.user._id, {
        isContact: true,
    ...
    Promise.all(promises).then(response => {
      this.contactIds = response[0].data.docs.map(user => {
        return user.connectedUser;
      });
      this.followingIds = response[1].data.docs.map(user => {
        return user.connectedUser;
      });
      this.followerIds = response[2].data.docs.map(user => {
        return user.connectedUser;
      });
      this.ready = true;
    });
