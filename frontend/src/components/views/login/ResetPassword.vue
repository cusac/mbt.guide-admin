          ...
          <div v-if="!passwordReset" class="col-md-4 col-md-offset-4">
            <vue-form :state="formstate" class="row">
              <validate
                auto-label
                class="form-group"
                :class="fieldClassName(formstate.newPassword)"
                :debounce="250"
              ...
              <validate
                ...
                auto-label
                class="form-group"
                :class="fieldClassName(formstate.pin)"
              ...
              <div class="content-centered">
                <button
                  ...
                  :disabled="formstate.$pristine || formstate.$invalid || passwordScoreUpdating"
                  @click="resetPassword()"
                >
                  Reset Password
                </button>

                <router-link v-if="error" to="/login">
...
export default {
  ...
  created() {
    ...
    if (!this.$route.query.token) {
      ...
      this.flashMessage =
        'The link you used did not contain a token. Please click on the reset password link in your email ' +
  ...
  methods: {
    ...
    resetPassword() {
      this.loading = true;
      authService
        .resetPassword(this.$route.query.token, this.newPassword, this.pin)
        .then(response => {
          this.loading = false;
        ...
        .catch(error => {
          ...
          if (error.data.message === 'Invalid PIN.') {
            this.flashMessage =
              'The PIN you provided is invalid. If you continue to have problems resetting your' +
              ' password please contact your administrator.';
          } else if (error.data.message === 'PIN required.') {
            this.flashMessage = error.data.message;
          } else {
            this.flashMessage =
              'There was an error resetting your password. The token in your email link may be expired. ' +
