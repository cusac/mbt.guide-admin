  ...
  >
    ...
    <div v-if="ready" class="box box-success direct-chat direct-chat-success">
      <div class="box-header with-border">
        <h3
          v-if="currentConversation && currentConversation.name"
          v-tooltip="userListTooltip(currentConversation.users)"
          class="box-title"
        >
          {{ currentConversation.name }}
        </h3>
        <h3 v-else-if="currentConversation" class="box-title">
          {{ currentConversation.users | userList }}
        </h3>
        <h3 v-else class="box-title">New Message</h3>
        <span v-if="currentConversation && currentConversation.chatType === CHAT_TYPES.GROUP"
          ><i class="fa fa-users"
        /></span>
        <div class="box-tools pull-right">
          ...
          <button v-tooltip="'Messages'" class="btn btn-box-tool" data-widget="chat-pane-toggle">
            <i class="fa fa-comments" />
          </button>
          <button class="btn btn-box-tool" @click="closeChat"><i class="fa fa-times" /></button>
        </div>
      </div>
      <!-- /.box-header -->
      <div ref="body" class="box-body">
        <!-- Search box for new messages -->
        <div v-if="newChat" ref="newChat" class="new-message-search">
          <vue-select
            ref="search"
        ...
        <div id="messages" class="direct-chat-messages">
          ...
          >
            <div class="direct-chat-info clearfix">
              <span class="direct-chat-name" :class="chatName(message)">{{
                displayName(message)
              }}</span>
              <span class="direct-chat-timestamp" :class="chatTimestamp(message)">{{
                message.createdAt | moment('MMM D, h:mm a')
              }}</span>
            </div>
            <!-- /.direct-chat-info -->
            <a href="#"
              ><img
                ...
                :src="userProfile(message)"
                alt="message user image"
                @click="goToProfile(message.user)"
            /></a>
            <!-- /.direct-chat-img -->
        ...
        <div class="direct-chat-contacts">
          <ul class="contacts-list">
            <li
              v-for="conversation in conversationsWithLast"
              :key="conversation._id"
              @click="getConversation({ conversation })"
            >
              <a href="#" data-widget="chat-pane-toggle">
                <img
                  v-if="conversation.lastMessage && conversation.chatType === CHAT_TYPES.GROUP"
                ...
                <div class="contacts-list-info">
                  ...
                  <span class="message-list-msg"
                    ...
                    }}{{ conversation.lastMessage.text | shortMessage(30) }}</span
                  >
                </div>
                <!-- /.contacts-list-info -->
              </a>
      ...
      <div class="box-footer chat-footer">
        <div class="input-group">
          ...
          <span class="input-group-btn">
            <button
              v-permission.enable="['postChatMessage']"
              :disabled="!currentConversation || !newMessageText"
              type="button"
...
export default {
  ...
  computed: {
    conversationsWithLast() {
      return this.conversations.filter(c => c.lastMessage);
    },
  },
  watch: {
  ...
  created() {
    ...
    eventBus.$on(EVENTS.MARK_CONVERSATION_AS_READ, conversation => {
      this.markConversation(conversation, true);
    });
    eventBus.$on(EVENTS.MARK_CONVERSATION_AS_UNREAD, conversation => {
  ...
  methods: {
    ...
    search: _.debounce((loading, search, vm) => {
      // TODO: firescrolling
      return vm.$userRepository
        .list({
          $term: search,
          $select: ['firstName', 'lastName', 'profileImageUrl'],
          $exclude: [vm.currentUser._id],
        })
        .then(response => {
          loading(false);
    ...
    markConversation(conversation, hasRead) {
      ...
      let promise = hasRead
        ...
        : chatService.markAsUnread(conversation._id);
    ...
    messageRecieved(message, flags) {
      ...
      if (message.user !== this.currentUser._id) {
        ...
        promise.then(conversation => {
          message.user = conversation.users.find(user => {
            return user._id === message.user;
          });
