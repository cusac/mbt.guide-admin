...
export default {
  ...
  created() {
    ...
    promises.push(
      this.$userRepository.getNotifications(this.currentUser._id, {
        $embed: ['actingUser'],
        $sort: ['-createdAt'],
      })
    );
    ...
    eventBus.$on(EVENTS.MARK_NOTIFICATION_AS_READ, notification => {
      this.markNotification(notification, true);
    });
    eventBus.$on(EVENTS.MARK_NOTIFICATION_AS_UNREAD, notification => {
      this.markNotification(notification, false);
    });
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    markAsRead(notification) {
      return this.$notificationRepository
        .update(notification._id, { hasRead: true })
        .catch(error => {
    ...
    markAsUnread(notification) {
      return this.$notificationRepository
        .update(notification._id, { hasRead: false })
        .catch(error => {
          console.error('Notification.markAsRead-error:', error);
      ...
      switch (notification.type) {
        case NOTIFICATION_TYPES.SHARED_DOCUMENT:
          notification.message =
            ...
            ' ' +
            notification.actingUser.lastName +
